...
const CAMPO =
  "rounded-lg border border-grafite/25 bg-white px-3 py-1.5 font-texto text-sm text-tinta outline-none transition-colors focus:border-turquesa focus:ring-2 focus:ring-turquesa/20";
const ROTULO = "flex flex-col gap-1 font-codigo text-[11px] font-medium uppercase tracking-wide text-grafite";

const LEGENDA: { cor: string; classe: string; texto: string }[] = [
  { cor: "turquesa", classe: "bg-turquesa", texto: "Criação e renovação" },
  { cor: "ambar", classe: "bg-ambar", texto: "Edição, mudança de faixa e avisos" },
  { cor: "carmim", classe: "bg-carmim", texto: "Desativação e falhas" },
];
...
export function FiltrosHistorico({
  ...
  function aplicar() {
    router.push(`/modulos/sc-20?${montarParams().toString()}`);
  }
  ...
  function limpar() {
    ...
    setDe("");
    setAte("");
    router.push("/modulos/sc-20?aba=historico");
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-wrap items-end gap-3 rounded-xl border border-grafite/15 bg-white p-4">
        ...
        <label className={ROTULO}>
          Evento
          <select value={evento} onChange={(e) => setEvento(e.target.value)} className={CAMPO}>
            <option value="">Todos</option>
            {NATUREZAS.map((n) => (
        ...
        <a
          href={`/modulos/sc-20/historico/relatorio?${montarParams().toString()}`}
      ...
      <div className="flex flex-wrap items-center gap-x-5 gap-y-1.5 px-1 font-texto text-xs text-grafite">
        ...
        {LEGENDA.map((l) => (
          <span key={l.cor} className="inline-flex items-center gap-1.5">
            <span aria-hidden="true" className={`h-2 w-2 rounded-full ${l.classe}`} />
            {l.texto}
          </span>
        ))}
      </div>
